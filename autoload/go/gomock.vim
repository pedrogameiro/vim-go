...
function! go#gomock#GoMock(...) abort
  let recv = ""
  let iface = ""
  let interactive = 0
  ...
  if a:0 == 2
    ...
    call go#util#EchoError('usage: GoMock {package} {interface}')
    return
  endif
  ...
  try
    ...
    let [result, err] = go#util#Exec(['gomock', '--directory', dirname, package, iface])
...
function! go#gomock#Complete(arglead, cmdline, cursorpos) abort
